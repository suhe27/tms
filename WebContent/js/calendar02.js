//选中时间标记----------------------------------------------------------------------
var show_time_checked=""; 
		
//控制日历的尺寸--------------------------------------------------------------------
document.write("<div id=meizzCalendarLayer style='position: absolute; z-index: 9999; width: 144; height: 219; display: none'>"); 
...
strIframe +="<td colspan=3 class=out title='"+ WebCalendar.regInfo +"'><a style='font-weight:normal;color: "+WebCalendar.wordColor +";text-decoration:;cursor:hand'  onclick='parent.returnDate_empty()' >清除</a>" +
        "&nbsp;<a style='font-weight:normal;color: "+WebCalendar.wordColor +";text-decoration:;cursor:hand'  onclick='parent.hiddenCalendar();return false;' >关闭</a></td></tr></table>"+ 
...
//时分秒的下拉选择框--------------------------------------------------------------------
strIframe += "<select name=tmpHourSelect onblur='parent.hiddenSelect(this)' style='z-index:1;position:absolute;top:194;left:30;display:none'"+ 
" onchange='parent.WebCalendar.thisHour =this.value; parent.hiddenSelect(this); parent.writeCalendar();'></select>"+ 
"<select name=tmpMinuteSelect onblur='parent.hiddenSelect(this)' style='z-index:1; position:absolute;top:194;left:67;display:none'"+ 
" onchange='parent.WebCalendar.thisMinute=this.value; parent.hiddenSelect(this); parent.writeCalendar();'></select>"+
"<select name=tmpSecondSelect onblur='parent.hiddenSelect(this)' style='z-index:1; position:absolute;top:194;left:104;display:none'"+ 
" onchange='parent.WebCalendar.thisSecond=this.value; parent.hiddenSelect(this); parent.writeCalendar();'></select>";

strIframe +=" <table><tr>";
strIframe +="<td ><input type='checkbox' title='点击此处选择是否显示时间' name='show_time' value='' "+show_time_checked+" onclick='parent._show_time(this);'><td>";
strIframe +="<td width='28%' style='color: #000080;' id=meizzHour title='点击此处选择时' onclick='parent.funHourSelect(parseInt(this.innerText, 10))'"+ 
" onmouseover='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'"+ 
" onmouseout='this.bgColor=parent.WebCalendar.lightColor; this.style.color=parent.WebCalendar.wordColor'></td>"+ 
" <td width='28%' style='color: #000080;' id=meizzMinute title='点击此处选择分' onclick='parent.funMinuteSelect(parseInt(this.innerText, 10))'"+ 
" onmouseover='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'"+ 
" onmouseout='this.bgColor=parent.WebCalendar.lightColor; this.style.color=parent.WebCalendar.wordColor'></td>"+
" <td width='28%' style='color: #000080;' id=meizzSecond title='点击此处选择秒' onclick='parent.funSecondSelect(parseInt(this.innerText, 10))'"+ 
" onmouseover='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'"+ 
" onmouseout='this.bgColor=parent.WebCalendar.lightColor; this.style.color=parent.WebCalendar.wordColor'></td></tr><table>";

strIframe +="</td></tr><table></form></body></html>"; 
...
//判断是否显示时间------------------------------------------------------------------
function _show_time(src){
	if(src.checked==true){
		WebCalendar.timeShow=true;
		show_time_checked="checked";
	}else{
...
{ 
var m = isNaN(parseInt(new Date().getHours(), 10)) ? new Date().getHours(): parseInt(WebCalendar.thisHour); 
...
{ 
var m = isNaN(parseInt(new Date().getMinutes(), 10)) ? new Date().getMinutes(): parseInt(WebCalendar.thisMinute); 
...
{ 
var m = isNaN(parseInt(new Date().getSeconds(), 10)) ? new Date().getSeconds(): parseInt(WebCalendar.thisSecond); 
...
var _h = WebCalendar.thisHour; 
var _m = WebCalendar.thisMinute; 
var _s = WebCalendar.thisSecond; 

WebCalendar.daysMonth[1] = (0==y%4 && (y%100!=0 || y%400==0)) ? 29 : 28; 
if (!(y<=9999 && y >= 1000 && parseInt(m, 10)>0 && parseInt(m, 10)<13 && parseInt(d, 10)>0 && parseInt(_h, 10)<24 && parseInt(_h, 10)>=0 && parseInt(_m, 10)<60 && parseInt(_s, 10)<60)){ 
alert("对不起，你输入了错误的日期！"); 
WebCalendar.thisYear = new Date().getFullYear(); 
WebCalendar.thisMonth = new Date().getMonth()+ 1; 
WebCalendar.thisDay = new Date().getDate(); 
...
_h = WebCalendar.thisHour; 
_m = WebCalendar.thisMinute; 
_s = WebCalendar.thisSecond;
...
WebCalendar.iframe.meizzHour.innerText = _h +" 时";
WebCalendar.iframe.meizzMinute.innerText = _m +" 分";
WebCalendar.iframe.meizzSecond.innerText = _s +" 秒";
...
function returnDate_empty() //根据日期格式等返回用户选定的日期 
{ 
