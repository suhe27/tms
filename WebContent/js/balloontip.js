...
function BalloonTip(id, width, height, caption, title, message, target, action) {
	this.id = id;
	this.title = title;
...
BalloonTip.prototype.hide = function () {
	if (this.onunload()) {
...
//消息卸载事件，可以重写
BalloonTip.prototype.onunload = function () {
	return true;
};
// 消息命令事件，要实现自己的连接，请重写它 
BalloonTip.prototype.oncommand = function () {
	//window.open(this.action,this.target);
	//this.hide(); 
}; 
// 消息显示方法 
BalloonTip.prototype.show = function () {
	...
	str += "<DIV style='BORDER-RIGHT: FFFFFF 1px solid; PADDING-RIGHT: 8px; BORDER-TOP:#66A3FF 1px solid; PADDING-LEFT: 8px; FONT-SIZE: 12px; PADDING-BOTTOM: 8px; BORDER-LEFT:#FFFFFF 1px solid; WIDTH: 100%; COLOR:#FFFFFF; PADDING-TOP: 8px; BORDER-BOTTOM:#FFFFFF 1px solid; HEIGHT: 100%'>";
				//str += "<FONT color=#EE0000>" + this.title + "</FONT><BR>" 
	str += "<BR><DIV style='WORD-BREAK: break-all;' align=left><label hidefocus=true id='btCommand'><FONT color=#EE0000>" + this.message + "</FONT></label></DIV>";
	str += "</DIV>";
	str += "</TD>";
	str += "</TR>";
	str += "</TABLE>";
...
// 设置速度方法
BalloonTip.prototype.speed = function (s) {
	var t = 10;
	try {
		t = praseInt(s);
	}
...
// 设置步长方法
BalloonTip.prototype.step = function (s) {
	var t = 2;
	try {
		t = praseInt(s);
	}
...
BalloonTip.prototype.rect = function (left, right, top, bottom) {
	try {
