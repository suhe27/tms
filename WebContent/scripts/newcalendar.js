...
function SelectDate(obj,strFormat,x,y)
{
 ...
 if(document.getElementById("ContainerPanel")==null){InitContainerPanel();}
    ...
    cal = (cal==null) ? new Calendar(by, ey, 1) : cal;    // 不用每次都初始化
															// 2006-12-03 修正
...
Calendar.prototype.draw = function() {
  ...
  // mvAry[mvAry.length] = ' <form name="calendarForm" style="margin: 0px;">';
	// //因 <form> 不能嵌套， 2006-12-01 由www.ttkc.net改用 Div
  mvAry[mvAry.length]  = '  <div name="calendarForm" style="margin: 0px;">';
  mvAry[mvAry.length]  = '    <table width="100%" border="0" cellpadding="0" cellspacing="1" style="font-size:12px;width=220px;">';
...
// 2009-03-03 添加的代码，放置时间的行
  ...
  mvAry[mvAry.length]  = '      <div align="center" style="padding:4px 4px 4px 4px;width:210px;background-color:' + calendar.colors["input_bg"] + ';">';
...
Calendar.prototype.show = function (dateObj, popControl) {
 ...
 this.bindSecond();
  ...
  this.panel.style.left = (xy.x + leftX)+ "px"; // 由www.ttkc.net 2007-02-11 修改 →
												// 加入自定义偏移量
  this.panel.style.top = (xy.y + topY + dateObj.offsetHeight) + "px";

  // 由www.ttkc.net 2006-06-25 修改 → 把 visibility 变为 display，并添加失去焦点的事件
