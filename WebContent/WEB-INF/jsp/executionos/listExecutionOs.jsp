...
<body class="mobile">
	...
	<display:table name="osList" class="mobile" id="row" style="width:100%;"
			cellpadding="0" cellspacing="0" requestURI="listExecutionOSs.action">
			<display:caption><thead>
			<tr  class="pageHeader">
			  		<th class="fixTitle" style="width: 5%;" >Row</th>
					<th class="fixTitle" style="width: 35%">OS Name</th>
			...
			<display:column  property="osName" title="osName" class="fixTitle1"/>
			<display:column  property="projectName" title="projectName" class="fixTitle1"/>				
...
<script type="text/javascript">
	function deleteOs(id, name) {
		if (confirm("Confirm delete Os '" + name + "'")) {
			window.location.href = "delExecutionOS.action?osId=" + id;
		}
	}
	function addOs() {
		MyWindow.OpenCenterWindowScroll('toAddExecutionOS.action', 'detail', 400,
				800);		
	}

	function updateOs(id) {
		MyWindow.OpenCenterWindowScroll(
				'toUpdateExecutionOS.action?osId=' + id, 'detail', 400, 800);
	}
</script>
</html>
