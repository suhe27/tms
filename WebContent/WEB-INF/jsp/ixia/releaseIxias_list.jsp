...
<html>
	...
	<body topmargin="0" leftmargin="0" class="mobile">
		...
		<display:table name="boardList" class="mobile" id="row" 
			cellpadding="0" cellspacing="0" pagesize="50" requestURI="releaseIxias_list.action" sort="external">
		...
		<c:if test="${row.stateId== 2}">
			...
			<display:column>
				<input size="100" name="boardList[${row_rowNum }].comments" value="${row.comments }"	type="text"></input>
			</display:column>
			<display:setProperty name="basic.show.header" value="false"/>
		</display:table>
		<div style="width:100%;height:25px;">
			<p:page	action="releaseIxias_list.action" beanName="#pageBean"	classForId="pagination-digg" activeClass="active" linkOffClass="link-off" totalPageClass="total-page">
				<p:param name="target1" value="#target"/>	
				<p:param name="boardType" value="1"/>
			</p:page>
		</div>
		<div style="width:100%;height:25px;border:1px gray dotted;">
			&nbsp;<input type="button" class="btn" value="Release Ixia ports" onclick="releaseIxias();"/>
		</div>
		</form>
	</body>
