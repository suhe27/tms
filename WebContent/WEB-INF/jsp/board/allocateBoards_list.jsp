...
<script type="text/javascript"
	src="<%=basePath%>scripts/newcalendar.js"></script>
</head>
<body topmargin="0" leftmargin="0" class="mobile">
<form method="post" name="boardform" id="boardform"><input
	type="hidden" name="currPage" value="${currPage }"></input> <display:table
	name="boardList" class="mobile" id="row" style="width:100%;"
	cellpadding="0" cellspacing="0" pagesize="50"
	requestURI="listBoards.action" sort="external">
	<display:caption>
		<thead>
			<tr class="pageHeader">
				<th style="width: 32px;">Row</th>
	...
	<display:column>
		<c:if test="${row.stateId == 1}">
			<input name="boardList[${row_rowNum-1}].startDate" type="text"
				id="startdate_${row_rowNum-1 }"
				onfocus="SelectDate(this,'yyyy-MM-dd hh:mm:ss',0,0);" size="20"
				maxlength="20" readonly="readonly" value="${row.startDate }" />
		</c:if>
		<c:if test="${row.stateId != 1}">
					${row.startDate}
				</c:if>
	...
	<display:column>
		<c:if test="${row.stateId == 1}">
			<input name="boardList[${row_rowNum-1}].endDate" type="text"
				id="enddate_${row_rowNum-1 }"
				onfocus="SelectDate(this,'yyyy-MM-dd hh:mm:ss',0,0);autoCompleteTime('startdate_${row_rowNum-1 }');"
				size="20" maxlength="20" readonly="readonly" value="${row.endDate }" />
		</c:if>
		<c:if test="${row.stateId != 1}">
					${row.endDate}
				</c:if>
	...
	<display:column>
		<input size="50" name="boardList[${row_rowNum-1}].comments"
			value="${row.comments }" type="text"></input>
	</display:column>
	<display:setProperty name="basic.show.header" value="false" />
</display:table>
<div style="width: 100%; height: 25px;"><p:page
	action="allocateBoardsList.action" beanName="#pageBean"
...
&nbsp;<input type="button" class="btn" value="Book Boards"
	onclick="applyBoards();"
/></div>
</form>
</body>
<script type="text/javascript">
	function updateBoard(id) {
		MyWindow.OpenCenterWindowScroll("toUpdateBoard.action?boardId=" + id,
				'detail', 600, 800);
	}
